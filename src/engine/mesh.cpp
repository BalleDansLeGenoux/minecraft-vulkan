    ...
    for (int x = 0; x < CHUNK_SIZE; x++) {
        for (int y = 0; y < CHUNK_SIZE; y++) {
            for (int z = 0; z < CHUNK_SIZE; z++) {
                ...
                // if (x+1 < CHUNK_SIZE && voxels[x+1][y][z].id == 0)
                //     addQuad({chunkPos.x*CHUNK_SIZE+x, chunkPos.y*CHUNK_SIZE+y, chunkPos.z*CHUNK_SIZE+z}, {1, 0, 0}, {0, 0});
                
                // if (x-1 >= 0 && voxels[x-1][y][z].id == 0)
                //     addQuad({chunkPos.x*CHUNK_SIZE+x, chunkPos.y*CHUNK_SIZE+y, chunkPos.z*CHUNK_SIZE+z}, {-1, 0, 0}, {0, 0});
                
                if (y+1 < CHUNK_SIZE && voxels[x][y+1][z].id == 0)
                    addQuad({chunkPos.x*CHUNK_SIZE+x, chunkPos.y*CHUNK_SIZE+y, chunkPos.z*CHUNK_SIZE+z}, {0, 1, 0}, {0, 0});
                
                // if (y-1 >= 0 && voxels[x][y-1][z].id == 0)
                //     addQuad({chunkPos.x*CHUNK_SIZE+x, chunkPos.y*CHUNK_SIZE+y, chunkPos.z*CHUNK_SIZE+z}, {0, -1, 0}, {0, 0});

                // if (z+1 < CHUNK_SIZE && voxels[x][y][z+1].id == 0)
                //     addQuad({chunkPos.x*CHUNK_SIZE+x, chunkPos.y*CHUNK_SIZE+y, chunkPos.z*CHUNK_SIZE+z}, {0, 0, 1}, {0, 0});

                // if (z-1 >= 0 && voxels[x][y][z-1].id == 0)
                //     addQuad({chunkPos.x*CHUNK_SIZE+x, chunkPos.y*CHUNK_SIZE+y, chunkPos.z*CHUNK_SIZE+z}, {0, 0, -1}, {0, 0});
            }
        }
    }
