...
void Allocator::alloc(void* p_data, uint32_t p_nbBlock, uint32_t p_offset) {
    ...
    BufferManager::copyBuffer(_staging.value().get().getBuffer(), _buffer.getBuffer(), size, 0, p_offset * _blockSize);
}

void Allocator::extractData(void* p_dst, uint32_t p_nbBlock, uint32_t p_offset) {
    uint32_t size = p_nbBlock * _blockSize;

    BufferManager::copyBuffer(_buffer.getBuffer(), _staging.value().get().getBuffer(), size, p_offset * _blockSize, 0);
